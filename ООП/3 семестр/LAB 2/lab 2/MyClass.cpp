...
namespace myNameSpaces
{
	...
	double MyMatrix::Row::operator[](int _index)
	{
		if (_index > maxCols)
		{
			throw logic_error("Class MyMatrix, column index exceeds row size");
		}
		return row[_index];
	}
	...
	const MyMatrix::Row MyMatrix::operator[](int _index)const
	{
		if (_index > cols)
		{
			throw logic_error("Class MyMatrix, row index exceeds cols size");
		}
...
//id в ошибках
//очитка даты только если разные размеры +
